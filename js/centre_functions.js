...
function renderAllStudents() {
    ...
    if (!saSearch || !saStatus || !saBatch) return;

    const search = saSearch.value.toLowerCase();
    const status = saStatus.value;
    ...
    const data = allStudentsList.filter(x =>
        ...
        (!status || x.status === status) && (!batchId || x.batchId === batchId)
    );
...
function clearStudentFilters() { 
    ...
    const saSt = document.getElementById('sa-status');
